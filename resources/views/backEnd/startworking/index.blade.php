...
@section('title','Startworking Manage')
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="button-list">
                                    @if($value->status == 1)
                                    <form method="post" action="{{route('startworkings.inactive')}}" class="d-inline"> 
                                    @csrf
                                    ...
                                    @else
                                    <form method="post" action="{{route('startworkings.active')}}" class="d-inline">
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">        
                                    ...
                                    <a href="{{route('startworkings.edit',$value->id)}}" class="btn btn-xs btn-primary waves-effect waves-light"><i class="fe-edit-1"></i></a>

                                    <form method="post" action="{{route('startworkings.destroy')}}" class="d-inline">        
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
    ...
    <div class="col-12">
        <div class="card cust_card create_card"  id="create">
            ...
            <div class="card-body form_card_body">
               <form action="{{route('startworkings.store')}}" method="POST" class=row data-parsley-validate=""  enctype="multipart/form-data">
                    @csrf
                    <div class="col-sm-12">
                    ...
                    <div class="col-sm-12 mb-3">
                        <div class="form-group">
                            <label for="iconimage" class="form-label">Arrow Image (Optional)</label>
                            <input type="file" class="form-control @error('iconimage') is-invalid @enderror" name="iconimage" value="{{ old('iconimage') }}"  id="iconimage">
                            @error('iconimage')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
