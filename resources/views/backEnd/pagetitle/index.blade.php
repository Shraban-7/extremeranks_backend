...
@section('title','Page Title Manage')
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="button-list">
                                    <!-- @if($value->status == 1)
                                    <form method="post" action="{{route('pagetitle.inactive')}}" class="d-inline"> 
                                    @csrf
                                    ...
                                    @else
                                    <form method="post" action="{{route('pagetitle.active')}}" class="d-inline">
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">        
                                    ...
                                    <a href="{{route('pagetitle.edit',$value->id)}}" class="btn btn-xs btn-primary waves-effect waves-light"><i class="fe-edit-1"></i></a>

                                    <!-- <form method="post" action="{{route('pagetitle.destroy')}}" class="d-inline">        
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
   ...
   <!--  <div class="col-12">
        <div class="card cust_card create_card"  id="create">
            ...
            <div class="card-body form_card_body">
               <form action="{{route('pagetitle.store')}}" method="POST" class=row data-parsley-validate=""  enctype="multipart/form-data">
                    @csrf
                    <div class="col-sm-12">
                        <div class="form-group mb-3">
                            <label for="pagename" class="form-label">Pagename *</label>
                            <input type="text" class="form-control @error('pagename') is-invalid @enderror" name="pagename" value="{{ old('pagename') }}" id="pagename" required="">
                            @error('pagename')
                    ...
                    <div class="col-sm-12">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" name="subtitle" value="{{ old('title') }}" id="subtitle" required="">
                            @error('title')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
