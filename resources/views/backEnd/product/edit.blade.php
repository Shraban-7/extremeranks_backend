        ...
        <div class="card cust_card">
            ...
            <div class="card_top">
                ...
                <div class="card_head_widget text_right">
                    <a href="{{route('products.index')}}" class="b_btn">Manage</a>
                </div>
            </div>
           </div>
            ...
            <div class="card-body form_card_body">
                <form action="{{route('products.update')}}" method="POST" class=row data-parsley-validate=""  enctype="multipart/form-data" name="editForm">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="service_id" id="service_id" class="form-control @error('service_id') is-invalid @enderror" required>
                                ...
                                @foreach($services as $key => $value)
                                    <option value="{{ $value->id }}" {{ $value->id == $edit_data->service_id ? 'selected' : '' }}>
                                        {{ $value->service_name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-sm-12">
                        <div class="form-group mb-3">
                            <label for="product_name" class="form-label">Produt Title *</label>
                            <input type="text" class="form-control @error('product_name') is-invalid @enderror" name="product_name" value="{{ $edit_data->product_name}}" id="product_name" required="">
                            @error('prodcut_name')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
...
<script type="text/javascript">
     document.forms['editForm'].elements['service_id'].value="{{$edit_data->service_id}}"
